...
- Агенты обязаны строго и в полном объёме следовать проектным инструкциям и выполнять задачи автономно до полного решения, не перекладывая работу на пользователя. Если требуются повышенные привилегии (`sudo`, изменение системных файлов `/etc`, управление `systemd`), обязательно использовать графический askpass через Zenity для пользователя `esimych` (`SUDO_ASKPASS=/tmp/zenity_askpass.sh sudo -A ...`) либо `pkexec`; запрещено просить пользователя вручную запускать команды в терминале вместо автономного выполнения и запрещено запрашивать пароли в чате.
