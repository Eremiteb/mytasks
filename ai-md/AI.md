...
- Агенты обязаны строго и в полном объёме следовать проектным инструкциям и выполнять задачи автономно до полного решения, не перекладывая работу на пользователя. Если требуются повышенные привилегии (`sudo`, изменение системных файлов `/etc`, управление `systemd`), использовать наиболее подходящий для текущей операционной системы и графического окружения инструмент аутентификации для пользователя `esimych` (`pkexec`, системный Polkit agent, `kdialog`, `zenity` через `SUDO_ASKPASS` и `sudo -A` или нативные средства ОС); запрещено просить пользователя вручную запускать команды в терминале вместо автономного выполнения и запрещено запрашивать пароли в чате.
...
- Дедупликация БД (`downloads`): `artist/title` — служебные нормализованные ключи, а исходные строки драйвера хранятся в `original_artist/original_title` и используются для отчётов. `normalize_db_text` = NFKC + `casefold` + только `str.isalnum()` (все разделители, пробелы и пунктуация отброшены), поэтому «Вася & Петя» == «Вася +Петя» == «ВасяПетя»; слова не удаляются — «Song (Remix)» != «Song». Ключ проверяется по ID `<site>_<track-id>` и по паре `artist/title` (уникальный индекс). Плюс дедупликация по содержимому: `save_response` возвращает `(bytes, paths, sha256_hex)`, движок сверяет хеш с колонкой `filehash`; при совпадении только что записанные копии удаляются с диска, а строка пишется с `filehash = NULL` (`INSERT OR IGNORE`) — ID не качается повторно и не нарушается частичный UNIQUE. Хеш проверяется после загрузки — экономит место, не трафик. Индексы: `UNIQUE(artist,title)` и **частичный** `UNIQUE(filehash) WHERE filehash IS NOT NULL`. При открытии БД строки перенормализуются, дубликаты `artist/title` и не-NULL `filehash` схлопываются к самой свежей, недостающие `original_artist`, `original_title` и `filehash` добавляются через `ALTER TABLE`. Старые исходные поля заполняются доступными до нормализации значениями `artist/title`; уже потерянные прежней версией регистр и пунктуация невосстановимы. Порядок в `_create_table` важен: оба индекса снимаются ДО перенормализующего `UPDATE` и дедупа и создаются заново ПОСЛЕ — при более широком ключе `UPDATE` иначе нарушит ещё живой UNIQUE. Менять `normalize_db_text` только с бэкапом рабочей `music_downloader.db` (миграция удаляет схлопнувшиеся строки). Разовый backfill `filehash` по уже лежащим на диске файлам (`_cloud/` + разложенная `/mnt/copy/Music/<папка>/`) — `state/music_downloader_hash_cleanup.py` (dry-run без флага, `--apply` с бэкапом и JSON-отчётом); при штатной работе строки с `filehash = NULL` (файл не найден) не досчитываются.
...
- `shazam_com`: чарт `/ru-ru/charts/top-200/world` запрашивать с проверенными Firefox-заголовками `CHART_HEADERS` — Chrome-профиль и `curl` без заголовков возвращали 405/204/чужую страницу. Shazam (Next.js App Router за Fastly) периодически отдаёт по адресу чарта HTTP 200 со страницей чужой песни (иной canonical, ~1.8 МБ вместо ~3.7 МБ), дефект держится до истечения TTL узла и виден не со всех маршрутов; полная HTML-страница и RSC-поток того же адреса ломаются независимо (наблюдались оба сочетания, и «оба сразу»). Поэтому `get_tracks` на каждой попытке пробует **два источника**: `parse_chart` (HTML: canonical + карточки `[data-test-id="songItem"]` с восстановлением SSR-вставок `$RS(...)`, ранги 1…200, 200 уникальных `adam_<id>`), затем при неудаче `parse_flight_chart` (RSC: заголовок `RSC: 1`, ответ `text/x-component`; те же данные регуляркой по `self.__next_f` — `"trackName"/"artistName"/…/song/<id>/`; ранг = порядок карточек, совпадает с DOM; те же ключи `adam_<id>`). Попытки — по `CHART_RETRY_DELAYS` (`15, 45, 90` с) с `Connection: close`. **Query-параметр-«антикэш» не добавлять** — отдельный ключ Fastly чаще указывает на битую копию. Легаси-JSON `cdn.shazam.com/shazam/v3/.../genre-global-chart-1` **не использовать** — он заморожен на данных начала 2024 г. `ERROR` пишется только после того, как оба источника промолчали на всех попытках, и означает временный сбой Shazam/CDN; на поломку разбора указывают `В карточке чарта…` / `Ожидался полный Top 200…` / `RSC-поток Shazam…` при живом чарте. Аудио искать в Sefon (`/search/?q=`) и LMusic (`/search?q=`, `lmusic.parse_tracks`) только при точном совпадении `_match_key` исполнителя и названия; превью Shazam/Apple не использовать.
...
- Если нужен `sudo`, использовать наиболее подходящий для операционной системы графический askpass/аутентификатор с явным пользователем `esimych`; не запрашивать пароль в чате и не сохранять его.
