...
**Проверить при следующем повторении**: содержимое `redis_logs_capture` в
логе того прогона — искать признаки OOM/fork-ошибок вокруг времени
зависания; если пусто — гипотеза с `mem_limit` отпадает, и правдоподобнее
разовая перегрузка после MariaDB-оптимизации.
