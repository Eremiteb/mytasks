...
## files_versions отключён осознанно (2026-09-01)

В отличие от `files_trashbin` (см. п.5 инцидента выше), приложение
`files_versions` в Nextcloud на `10.66.66.1` отключено по явному решению
пользователя (`occ app:disable files_versions`), а не является багом —
версии файлов сознательно не хранятся. Соответственно шаг `occ
versions:cleanup` в `cloud_backup_qnap.sh` (был между очисткой корзин и
оптимизацией БД) убран полностью, а не просто игнорирует ошибку: при
выключенном приложении у occ нет самого namespace `versions`, вызов —
это не «мягкая» ошибка, а ожидаемое отсутствие команды. Лог теперь пишет
`occ_versions_skipped` вместо `occ_cleanup_versions_ok/failed`.

Если `files_versions` когда-нибудь снова понадобится — просто
`occ app:enable files_versions`; шаг очистки версий в скрипте придётся
вернуть отдельно (сам он не отслеживает состояние приложения).
