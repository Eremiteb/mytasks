...
## Graphify-граф проекта

Для навигации по функциям, вызовам и связям между скриптами используется
локальный Graphify-граф. Он генерируется в каталоге `graphify-out/`, который
добавлен в `.gitignore` и не включается в коммиты.

Основные артефакты:

- `graphify-out/graph.json` — машиночитаемый граф;
- `graphify-out/graph.html` — интерактивная визуализация;
- `graphify-out/GRAPH_REPORT.md` — архитектурная сводка.

Текущий граф строится в локальном режиме только по коду, без отправки файлов
во внешние LLM/API. После изменения кода обновите его штатными командами:
...
Перед ручным поиском по коду следует сначала использовать существующий граф
через `graphify query`, `graphify explain` или `graphify path`.
