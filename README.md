...
**Важно:** в строке crontab вызывайте скрипт напрямую (как в примере выше), не через `sh /share/Public/tasks/cloud_backup_qnap.sh` — у скрипта шебанг `#!/opt/bin/bash`, а явный `sh script.sh` его игнорирует и выполняет под системным `/bin/sh` QTS (это не bash — самостоятельный бинарник, а не symlink), для которого скрипт не писался. На практике это уже приводило к разовому необъяснимому `ncat: command not found` (rc=127) в `measure_network_speed`, хотя PATH и бинарь были на месте.
