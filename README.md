...
Папки и лимиты проверяются один раз до обращения к сайтам. Отсутствующий/пустой глобальный список, отсутствующая папка или достигнутый лимит завершают загрузчик с кодом `2`, без запуска сортировщика. Это стартовая проверка, а не жёсткая квота во время скачивания. Ошибки отдельных сайтов и треков логируются, но не обязательно меняют итоговый код выхода. При отсутствии файла конфигурации текущий загрузчик возвращает `0` без скачивания — подготовьте JSON до запуска обёртки. Пути сортировщика настраиваются отдельно.
